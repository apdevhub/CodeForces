/* LinkedIn :: aruppatra */
/* Codeforces :: arup-04 */
/* Website :: https://arup-patra.vercel.app/ */
...
public class A_2072 {
    ...
    static void solve() throws IOException {
        ...
        int k = nextInt();
        int p = nextInt();
        System.out.println(findMinStep(n, k, p));
    }

    // new create function for this solution
    public static int findMinStep(int n, int k, int p){
        if(k == 0){
            return 0;
        }
        int req = Math.abs(k/p);
        if(k%p != 0) req++;
        return Math.abs(req)<=n? Math.abs(req) : -1;
    }
